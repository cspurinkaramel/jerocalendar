...
// --- The Visionary Interface: Direct PDF/Image Analysis ---
const VISION_SYSTEM_PROMPT = `お前は優秀なスケジュール解析AIだ。
提供された画像（プリントや工程表など）に記載されている全ての日程・予定・タスクを抽出し、以下のJSON配列形式のみで出力せよ。マークダウン等の装飾は一切不要。
現在の年は {{CURRENT_YEAR}} 年だ。年が省略されている場合はこれを補え。
...
[
  {
    ...
    "start": "YYYY-MM-DD",
    "end": "YYYY-MM-DD",
    "description": "備考や詳細があれば"
  }
]`;

async function analyzeVisionData(base64String) {
    if (!navigator.onLine) {
        showToast('電波がない。PDF解析には私の頭脳(クラウド)への接続が必要だ。');
        return;
    }
    const apiKey = (localStorage.getItem('jero_gemini_key') || "").trim();
    if(!apiKey) { showToast('設定からGemini APIキーを入力してくれ。'); return; }

    showGlobalLoader('ジェロが画像を解析中... (10〜20秒待て)');

    const currentYear = new Date().getFullYear();
    const sysPrompt = VISION_SYSTEM_PROMPT.replace('{{CURRENT_YEAR}}', currentYear);
    ...
    const payload = {
        ...
        contents: [{
            role: "user",
            parts: [
                { text: "この画像からすべての予定を抽出し、指定のJSON配列形式で出力しろ。" },
    ...
    try {
        ...
        if (!Array.isArray(actions) || actions.length === 0) {
            showToast('予定が見つからなかった、あるいは解析に失敗した。');
            return;
        }

        showGlobalLoader(`解析完了。${actions.length}件の予定を自動登録中...`);

        // 抽出された予定を一括でカレンダー（Google API / IDB）へ叩き込む
        for (const action of actions) {
            // startと同じ日をendに設定（終日予定の形式）
        ...
        showToast(`✅ ${actions.length}件の予定をカレンダーに追加したぞ。`);
        
        // 最初の予定の月へ画面をリロードして反映させる
        if(actions[0] && actions[0].start) {
            const fd = new Date(actions[0].start);
            await fetchAndRenderMonth(fd.getFullYear(), fd.getMonth(), 'replace', true);
        } else {
            triggerFullReRender();
        }
