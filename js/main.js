    ...
    try {
        ...
        while (navigator.onLine && !authErrorOccurred) {
            ...
            for (let item of queue) {
                ...
                // ★絶対防壁1：前回の通信がタイムアウトで終わった疑いがある場合、必ずサーバーの最新状態を確認(Fetch)してから処理する
                if (localStorage.getItem('jero_needs_verify') === 'true') {
                    if (!isSilent) showToast('🔍 サーバーとのデータ整合性を確認中...');
                    await fetchAndRenderMonth(td.getFullYear(), td.getMonth(), 'replace', true);
                    localStorage.removeItem('jero_needs_verify');
                }

                // ★絶対防壁2：ドッペルゲンガー迎撃（Pre-flight Check）
                let isGhostAlreadyReal = false;
                if (action.method === 'insert') {
                    const cache = dataCache[monthKey];
                    if (cache) {
                        if (action.type === 'event' && cache.events) {
                            isGhostAlreadyReal = cache.events.some(e => e.id && !e.id.startsWith('dummy_') && e.summary === action.title && (e.start.date === action.start || e.start.dateTime === action.start));
                        } else if (action.type === 'task' && cache.tasks) {
                            isGhostAlreadyReal = cache.tasks.some(t => t.id && !t.id.startsWith('dummy_') && t.title === action.title && t.due === action.due);
                        }
                    }
                }

                if (isGhostAlreadyReal) {
                    console.log("👻 幻影重複を検知。既にサーバーに存在するため退避キューから抹消:", action.title);
                ...
                // ★絶対防壁3：盲目的な連続リトライを禁止する（1回のキュー処理につき1回勝負）
                let retries = 1; 
                ...
                while (retries > 0 && !itemSuccess && !authErrorOccurred) {
                    try {
                        ...
                        if (code === 401 || code === 403) { authErrorOccurred = true; hideGlobalLoader(); showToast('⚠️ サーバー側で認証エラーが起きた。'); await updateSyncBadge(); } 
                            ...
                            // ★絶対防壁4：タイムアウト時は次回の検証フラグを立てて、キューの処理全体を即座に停止する
                            if (code === 408 || msg.includes('タイムアウト') || msg.includes('Offline')) {
                                localStorage.setItem('jero_needs_verify', 'true');
                                break;
                            }
