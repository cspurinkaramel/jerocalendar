// JeroCalendar v9.3 Main Logic - Full Drive Nexus (Multiple) Edition
function getGasUrl() {
...
async function processSyncQueue() {
    ...
    const queue = await getSyncQueue();
    if (queue.length === 0) { await updateSyncBadge(); return; }
    ...
    for (let item of queue) {
        ...
        while (retries > 0 && !itemSuccess && !authErrorOccurred) {
            try {
                ...
                successCount++; itemSuccess = true; needsRefresh = true;
                ...
                if (code === 401 || code === 403) {
                    authErrorOccurred = true; hideGlobalLoader(); showToast('⚠️ サーバー側で認証エラーが起きた。'); await updateSyncBadge();
                } else if (code === 400 || code === 404 || code === 410) {
                    await clearSyncQueueItem(item.id); itemSuccess = true; needsRefresh = true;
                } else if (code === 429) {
                    authErrorOccurred = true; showToast('⚠️ Googleの通信制限に引っかかった。');
                } else {
                    retries--;
...
function isEventSpanning(eventObj, dateStr) {
    ...
    if (eventObj.start.date && eventObj.end.date) {
        stDateStr = eventObj.start.date; const edDate = new Date(eventObj.end.date); edDate.setDate(edDate.getDate() - 1); edDateStr = `${edDate.getFullYear()}-${String(edDate.getMonth() + 1).padStart(2, '0')}-${String(edDate.getDate()).padStart(2, '0')}`;
    } else if (eventObj.start.dateTime && eventObj.end.dateTime) {
        const stD = new Date(eventObj.start.dateTime); stDateStr = `${stD.getFullYear()}-${String(stD.getMonth() + 1).padStart(2, '0')}-${String(stD.getDate()).padStart(2, '0')}`;
        const edD = new Date(eventObj.end.dateTime); if (edD.getHours() === 0 && edD.getMinutes() === 0) { edD.setDate(edD.getDate() - 1); }
    ...
    if (dateStr === stDateStr) return 'span-start'; if (dateStr === edDateStr) return 'span-end';
    if (dateStr > stDateStr && dateStr < edDateStr) return 'span-mid'; return 'single';
}
...
async function openDailyModal(dateStr, dow) {
    ...
    const monthKey = `${y}-${parseInt(m) - 1}`; const data = dataCache[monthKey]; let hasItems = false; let modalItems = [];
    if (data) {
...
function renderMonthDOM(year, month, data, position) {
    ...
    const today = new Date(); const slotMap = {};
    for (let i = 1; i <= daysInMonth; i++) { const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`; slotMap[dateStr] = []; }

    sortedEvents.forEach(e => {
        if (!e.start) return; const occupiedDates = [];
        ...
        while (true) {
            let isFree = true;
            for (const d of occupiedDates) { if (slotMap[d][slotIndex]) { isFree = false; break; } }
            if (isFree) break; slotIndex++;
        }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        slots.forEach(e => {
            ...
            const isPendingInsert = e._localId ? true : false; const isPendingUpdate = e._pendingUpdate ? true : false; const isPendingDelete = e._pendingDelete ? true : false;
            let stateIcon = ''; if (isPendingInsert) stateIcon = '➕🔄 '; if (isPendingUpdate) stateIcon = '📝🔄 '; if (isPendingDelete) stateIcon = '🗑️ ';
            const recurIcon = e.recurrence ? '🔁 ' : ''; const pData = processSemanticText(e.summary);
            div.innerText = stateIcon + recurIcon + pData.text + (timeStr ? ` (${timeStr})` : '');
            let bgColor = 'var(--accent)'; let txtColor = '#ffffff';
            if (pData.style) { bgColor = pData.style.bg; txtColor = pData.style.txt; } else if (e.colorId && GOOGLE_COLORS[e.colorId]) { bgColor = GOOGLE_COLORS[e.colorId]; txtColor = getContrastYIQ(bgColor); }
            ...
            if (isPendingInsert || isPendingUpdate) { div.style.border = `1px dashed ${txtColor}`; div.style.opacity = '0.8'; }
            if (isPendingDelete) { div.style.textDecoration = 'line-through'; div.style.opacity = '0.3'; div.style.filter = 'grayscale(100%)'; }
        ...
        if (data.tasks) data.tasks.filter(t => t.due && t.due.includes(dateStr)).forEach(t => {
            ...
            const isPendingInsert = t._localId ? true : false; const isPendingDelete = t._pendingDelete ? true : false; const insertIcon = isPendingInsert ? '➕🔄 ' : ''; const deleteIcon = isPendingDelete ? '🗑️ ' : '';
            ...
            div.style.height = '14px'; div.style.lineHeight = '14px'; div.style.margin = '1px 2px'; div.style.padding = '0 3px'; div.style.borderRadius = '3px'; div.style.fontSize = '10px'; div.style.boxSizing = 'border-box';
            if (isPendingInsert) { div.style.border = `1px dashed var(--txt)`; div.style.opacity = '0.8'; }
            if (isPendingDelete) { div.style.textDecoration = 'line-through'; div.style.opacity = '0.4'; div.style.filter = 'grayscale(100%)'; }
            dayEl.appendChild(div);
        });
...
async function initCalendar() {
    ...
    setProgress(10);
    try {
        await loadDataCacheFromIDB(); await rehydrateSyncQueue();
        const today = new Date(); const y = today.getFullYear(); const m = today.getMonth();
        await fetchAndRenderMonth(y, m, 'append', false); await fetchAndRenderMonth(y, m + 1, 'append', false);
        scrollToToday();
        if (navigator.onLine && !isAuthError) { fetchAndRenderMonth(y, m, 'replace', true); fetchAndRenderMonth(y, m + 1, 'replace', true); }
        await updateSyncBadge();
...
async function fetchAndRenderMonth(year, month, position = 'append', forceFetch = false) {
    ...
    if (forceFetch || !dataCache[monthKey]) {
        ...
        let events = [], tasks = [];
        try {
            const url = `${getGasUrl()}?year=${year}&month=${month}`; const response = await fetch(url); const data = await response.json();
            if (data.success) { events = data.events || []; tasks = data.tasks || []; } else { throw new Error(data.error || '不明なサーバーエラー'); }
        } catch (e) { showToast('通信エラーが発生した。'); return; }
...
function renderIconPalette(targetId, inputId) {
    ...
    advancedDict.forEach(item => {
        ...
        const prefix = item.keys[0]; const btn = document.createElement('div');
        ...
        btn.onclick = () => { const inputEl = document.getElementById(inputId); if (!inputEl.value.startsWith(prefix)) { inputEl.value = prefix + " " + inputEl.value; } };
...
function openEditor(e = null) {
    ...
    const alldayToggle = document.getElementById('edit-allday'); alldayToggle.checked = !!isAllDay;
    ...
    if (e && e.start) { st = new Date(e.start.dateTime || e.start.date); ed = new Date(e.end.dateTime || e.end.date); if (isAllDay) ed.setDate(ed.getDate() - 1); }
    startInput.type = isAllDay ? 'date' : 'datetime-local'; endInput.type = isAllDay ? 'date' : 'datetime-local';
    if (isAllDay) { startInput.value = getSafeLocalDateStr(st); endInput.value = getSafeLocalDateStr(ed); }
    else { const tzOffset = st.getTimezoneOffset() * 60000; startInput.value = new Date(st.getTime() - tzOffset).toISOString().slice(0, 16); endInput.value = new Date(ed.getTime() - tzOffset).toISOString().slice(0, 16); }
    document.getElementById('editor-title').innerText = e ? '予定の編集' : '新規予定';
    document.getElementById('btn-delete').style.display = e ? 'block' : 'none'; document.getElementById('btn-duplicate').style.display = e ? 'block' : 'none';
    const convertBtn = document.getElementById('btn-convert-task'); if (convertBtn) convertBtn.style.display = e ? 'block' : 'none';
...
async function saveEvent() {
    ...
    if (activeEventUrls.length > 0) { finalDesc += '\n\n📁 添付ファイル:\n' + activeEventUrls.join('\n'); }

    const action = { type: 'event', method: id ? 'update' : 'insert', id: id, title: title, location: document.getElementById('edit-loc').value, description: finalDesc, colorId: selectedColorId };
    if (pendingEventAttachments.length > 0) action.attachments = pendingEventAttachments;

    try {
        if (isAllDay) {
            action.start = startVal; let parts = endVal.split('-');
            const ed = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2])); ed.setDate(ed.getDate() + 1);
            action.end = getSafeLocalDateStr(ed);
...
function openTaskEditor(t = null) {
    ...
    const parsed = parseAttachments(t ? t.notes || '' : '');
    let cleanNotes = parsed.cleanText;
    activeTaskUrls = parsed.urls;
    ...
    if (t && t.notes) { const extracted = extractTaskData(cleanNotes); cleanNotes = extracted.cleanNotes; selectTaskColor(null, extracted.colorId); } else { selectTaskColor(null, ''); }
...
function closeTaskEditor() { 
    ...
    if(prev) prev.innerHTML = ''; 
    pendingTaskAttachments = []; activeTaskUrls = [];
}
...
async function toggleTaskCompletion(taskId, newStatus) {
    ...
    try {
        if (navigator.onLine) { await fetch(getGasUrl(), { method: 'POST', body: JSON.stringify(payload) }); } 
        else { showToast('圏外ではタスクの完了操作はできない。'); targetTask.status = newStatus === 'completed' ? 'needsAction' : 'completed'; await fetchAndRenderMonth(td.getFullYear(), td.getMonth(), 'replace', false); }
...
async function saveTask() {
    ...
    if (activeTaskUrls.length > 0) { rawNotes += '\n\n📁 添付ファイル:\n' + activeTaskUrls.join('\n'); }
...
async function executeConversion(fromType) {
    ...
    if (fromType === 'event') {
        ...
        else { insertAction.start = getSafeLocalDateStr(); const tmrw = new Date(); tmrw.setDate(tmrw.getDate() + 1); insertAction.end = getSafeLocalDateStr(tmrw); }
    }
    ...
    (async () => {
        try {
            if (navigator.onLine) { 
                if (deleteAction) await executeApiAction(deleteAction); await executeApiAction(insertAction); showToast('✅ 変換を完了した'); await fetchAndRenderMonth(redrawDate.getFullYear(), redrawDate.getMonth(), 'replace', true);
            } else { 
                if (deleteAction) await saveToSyncQueue(deleteAction); await saveToSyncQueue(insertAction); showToast('📦 圏外のためローカルの控え室に保管した。'); await updateSyncBadge();
            }
        } catch (e) { showToast('❌ 変換中に通信エラー発生。控え室を確認しろ。'); if (deleteAction) await saveToSyncQueue(deleteAction); await saveToSyncQueue(insertAction); await updateSyncBadge(); }
    })();
}
...
async function dispatchManualAction(action) {
    ...
    if (tdStr && typeof tdStr === 'string') { 
        if (tdStr.includes('T')) { td = new Date(tdStr); } else { const p = tdStr.split('-'); td = new Date(parseInt(p[0]), parseInt(p[1]) - 1, parseInt(p[2])); } 
    }
    const year = td.getFullYear(); const month = td.getMonth(); const monthKey = `${year}-${month}`;
    ...
    (async () => {
        if (!navigator.onLine) {
            const localId = await saveToSyncQueue(action); updateLocalCacheForOptimisticUI(action, localId, tempLocalId); showToast(`📦 圏外だ。${msgAction}を控え室に退避した。`); await updateSyncBadge();
        } else {
            try {
                ...
                if (dataCache[monthKey]) {
                    if (action.method === 'delete') {
                        ...
                        let targetList = action.type === 'event' ? dataCache[monthKey].events : dataCache[monthKey].tasks;
                        let existing = targetList.find(e => e.id === action.id); if (existing) delete existing._pendingUpdate;
                    } else if (action.method === 'insert') {
                        for (const key in dataCache) {
                            if (action.type === 'event' && dataCache[key].events) { const existing = dataCache[key].events.find(e => e._localId === tempLocalId); if (existing) delete existing._localId; }
                            if (action.type === 'task' && dataCache[key].tasks) { const existing = dataCache[key].tasks.find(t => t._localId === tempLocalId); if (existing) delete existing._localId; }
                        }
                        setTimeout(() => fetchAndRenderMonth(year, month, 'replace', true), 1500);
                    }
                }
                const existingMonthAfter = document.getElementById(`month-${year}-${month}`); if (existingMonthAfter) existingMonthAfter.remove();
                if (dataCache[monthKey]) renderMonthDOM(year, month, dataCache[monthKey], 'replace');
                if (typeof selectedDateStr !== 'undefined' && selectedDateStr) openDailyModal(selectedDateStr, new Date(selectedDateStr).getDay()); await updateSyncBadge();
            } catch (e) {
                const localId = await saveToSyncQueue(action); updateLocalCacheForOptimisticUI(action, localId, tempLocalId); showToast(`📦 通信不良だ。裏で控え室に退避した。`);
                const existingMonthErr = document.getElementById(`month-${year}-${month}`); if (existingMonthErr) existingMonthErr.remove();
                if (dataCache[monthKey]) renderMonthDOM(year, month, dataCache[monthKey], 'replace');
                if (typeof selectedDateStr !== 'undefined' && selectedDateStr) openDailyModal(selectedDateStr, new Date(selectedDateStr).getDay()); await updateSyncBadge();
            }
        }
...
function updateLocalCacheForOptimisticUI(action, localId, replaceTempId = null) {
    ...
    if (action.method === 'insert') {
        ...
        const newItem = action.type === 'event' ? { id: 'dummy_' + localId, summary: action.title, location: action.location, description: action.description, start: action.start.includes('T') ? { dateTime: action.start } : { date: action.start }, end: action.end ? (action.end.includes('T') ? { dateTime: action.end } : { date: action.end }) : null, colorId: action.colorId, _localId: localId } : { id: 'dummy_' + localId, title: action.title, notes: action.description, due: action.due, status: 'needsAction', _localId: localId };
        targetList.push(newItem);
    } else if (action.method === 'update') {
        const existing = targetList.find(item => item.id === action.id);
        if (existing) {
            if (action.type === 'event') { existing.summary = action.title; existing.location = action.location; existing.description = action.description; existing.start = action.start.includes('T') ? { dateTime: action.start } : { date: action.start }; if (action.end) existing.end = action.end.includes('T') ? { dateTime: action.end } : { date: action.end }; existing.colorId = action.colorId; } 
            else { existing.title = action.title; existing.notes = action.description; existing.due = action.due; }
...
async function executeApiAction(action, isRetry = false) {
    ...
    payload.title = payload.title || "(無名)"; payload.description = payload.description || ""; payload.location = payload.location || "";
    ...
    if (payload.type === 'event') {
        ...
        if (payload.due && typeof payload.due === 'string') { const dateMatch = payload.due.match(/^(\d{4}-\d{2}-\d{2})/); if (dateMatch) { payload.due = dateMatch[1] + 'T00:00:00.000Z'; } }
    }

    try {
        const response = await fetch(getGasUrl(), { method: 'POST', body: JSON.stringify(payload) }); const result = await response.json();
        if (!result.success) {
            let simulatedStatus = 500; const errStr = (result.error || "").toLowerCase();
            if (errStr.includes("not found") || errStr.includes("404")) simulatedStatus = 404; else if (errStr.includes("invalid") || errStr.includes("400") || errStr.includes("bad request") || errStr.includes("parse")) simulatedStatus = 400; else if (errStr.includes("410") || errStr.includes("gone")) simulatedStatus = 410; else if (errStr.includes("429") || errStr.includes("quota") || errStr.includes("rate limit")) simulatedStatus = 429; else if (errStr.includes("401") || errStr.includes("403") || errStr.includes("unauthorized") || errStr.includes("forbidden")) simulatedStatus = 401;
...
window.addEventListener('online', async () => { showToast('📶 電波が回復した。'); await updateSyncBadge(); processSyncQueue(); });
window.addEventListener('offline', async () => { showToast('⚡️ 圏外になった。以後の操作はローカルの控え室に退避する。'); await updateSyncBadge(); });
...
async function executeSilentRefresh() {
    ...
    const monthsToRefresh = [...renderedMonths]; if (monthsToRefresh.length === 0) return;
    let isUpdated = false;
    for (const m of monthsToRefresh) {
        try {
            const url = `${getGasUrl()}?year=${m.year}&month=${m.month}`; const response = await fetch(url); const data = await response.json();
            if (data.success) {
                const monthKey = `${m.year}-${m.month}`; const oldDataStr = JSON.stringify(dataCache[monthKey]); const newDataStr = JSON.stringify({ events: data.events || [], tasks: data.tasks || [] });
                if (oldDataStr !== newDataStr) {
                    dataCache[monthKey] = { events: data.events || [], tasks: data.tasks || [] }; saveDataCacheToIDB(monthKey, dataCache[monthKey]);
                    const existingMonth = document.getElementById(`month-${m.year}-${m.month}`);
                    if (existingMonth) { existingMonth.remove(); renderMonthDOM(m.year, m.month, dataCache[monthKey], 'replace'); }
                    isUpdated = true;
                }
            }
        } catch (e) { console.warn("サイレント同期スキップ:", e); }
    }
    if (isUpdated && typeof selectedDateStr !== 'undefined' && selectedDateStr) { const modal = document.getElementById('daily-modal'); if (modal && modal.classList.contains('active')) { openDailyModal(selectedDateStr, new Date(selectedDateStr).getDay()); } }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (localStorage.getItem('jero_token')) { setTimeout(() => { if (!isCalendarInited) { document.getElementById('offline-badge').innerText = '⚡️ 完全自律モード (キャッシュ起動)'; document.getElementById('offline-badge').classList.add('active'); initCalendar(); } }, 1500); }
...
document.addEventListener('DOMContentLoaded', () => {
    const resizer = document.getElementById('resizer'); const bottomView = document.getElementById('bottom-detail-view'); let startY = 0; let startHeight = 0;
    if (resizer && bottomView) {
        resizer.addEventListener('touchstart', (e) => { startY = e.touches[0].clientY; startHeight = bottomView.getBoundingClientRect().height; document.body.style.userSelect = 'none'; }, { passive: true });
        document.addEventListener('touchmove', (e) => { if (startY === 0) return; const deltaY = startY - e.touches[0].clientY; let newHeight = startHeight + deltaY; const minH = window.innerHeight * 0.1; const maxH = window.innerHeight * 0.7; if (newHeight < minH) newHeight = minH; if (newHeight > maxH) newHeight = maxH; bottomView.style.height = `${newHeight}px`; }, { passive: true });
        document.addEventListener('touchend', () => { startY = 0; document.body.style.userSelect = ''; });
    }
});
...
async function renderSyncQueueList() {
    const listEl = document.getElementById('sync-queue-list'); if (!listEl) return;
    const queue = await getSyncQueue(); if (queue.length === 0) { listEl.innerHTML = '<div style="text-align:center; padding:20px; color:#888; font-size:13px;">未送信のデータはない。平和だ。</div>'; return; }
    let html = '';
    queue.forEach(item => {
        const payload = item.payload; const isEvent = payload.type === 'event'; const method = payload.method === 'insert' ? '追加' : payload.method === 'update' ? '更新' : '削除'; const title = payload.title || '(無名)';
        let dateStr = "日時不明"; if (payload.start) dateStr = payload.start.includes('T') ? new Date(payload.start).toLocaleString('ja-JP') : payload.start; else if (payload.due) dateStr = new Date(payload.due).toLocaleDateString('ja-JP');
        html += `<div style="background:var(--head-bg); border:1px solid var(--border); border-radius:8px; padding:10px; display:flex; justify-content:space-between; align-items:center;"><div style="flex:1; overflow:hidden; margin-right:10px;"><div style="font-size:10px; color:var(--accent); font-weight:bold; margin-bottom:2px;">[${isEvent ? '予定' : 'タスク'} : ${method}]</div><div style="font-size:14px; font-weight:bold; color:var(--txt); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${title}</div><div style="font-size:11px; color:#888;">${dateStr}</div></div><div style="display:flex; gap:6px; flex-shrink:0;"><button class="btn-gray" style="padding:6px 10px; font-size:12px; border-radius:6px; border:none; color:white; cursor:pointer;" onclick="retrySingleSyncItem('${item.id}')">再送</button><button class="btn-red" style="padding:6px 10px; font-size:12px; border-radius:6px; border:none; color:white; cursor:pointer;" onclick="discardSingleSyncItem('${item.id}')">破棄</button></div></div>`;
    });
    listEl.innerHTML = html;
}
async function discardSingleSyncItem(id) { if (!confirm("この未送信データを破棄する。ゴーストデータならこれが正解だ。いいか？")) return; await clearSyncQueueItem(id); await renderSyncQueueList(); await updateSyncBadge(); showToast("🚮 データを破棄した。UIのゴーストを浄化する。"); const today = new Date(); showGlobalLoader("カレンダーを最新化中..."); await fetchAndRenderMonth(today.getFullYear(), today.getMonth(), 'replace', true); hideGlobalLoader(); }
async function retrySingleSyncItem(id) {
    const queue = await getSyncQueue(); const item = queue.find(q => q.id === id); if (!item) return;
    showGlobalLoader("1件だけ再送信中...");
    try { await executeApiAction(item.payload); await clearSyncQueueItem(id); showToast("✅ 送信成功だ！"); await renderSyncQueueList(); await updateSyncBadge(); const today = new Date(); await fetchAndRenderMonth(today.getFullYear(), today.getMonth(), 'replace', true); } 
    catch (e) { showToast("❌ やはり弾かれた。形式が不正なゴーストデータの可能性が高い。破棄を勧めるぞ。"); } finally { hideGlobalLoader(); }
}